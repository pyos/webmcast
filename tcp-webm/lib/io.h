...
namespace aio
{
    ...
    struct server
    {
        ...
        int               const fd;
        bool              const owns_fd = false;
        bool ok = false;

        server(evloop *loop, protocol::factory pf, int fd) : loop(loop), new_proto(pf), fd(fd)
        {
            loop->read[fd] += &on_accept;
            ok = true;
        }

        server(evloop *loop, const server *other) : server(loop, other->new_proto, other->fd)
        {
            // `accept` will succeed on a random server.
        }

        server(evloop *loop, protocol::factory pf, uint32_t iface, uint32_t port)
            : loop      (loop)
            , new_proto (pf)
            ...
            , owns_fd   (fd >= 0)
        {
            ...
            if (-1 == setsockopt (fd, SOL_SOCKET, SO_REUSEADDR, &aio::ONE, sizeof(aio::ONE))
       ...
       ~server()
        ...
        const evloop::callback on_accept = [this]()
        {
            ...
            int fd = accept4(this->fd, (struct sockaddr *) &addr, &size, SOCK_NONBLOCK);
            if (fd < 0)
                return -1;

            if (fd > 1023 || -1 == setsockopt(fd, IPPROTO_TCP, TCP_NODELAY, &ONE, sizeof(ONE)))
            {
                ::close(fd);
                return -1;
            }
