...
$.emoji = () => {
    let tooltip = document.createElement('x-tooltip');
    let root = document.createElement('x-emoji-picker');
    let tabs = document.createElement('x-tabbar');
    let wrap = document.createElement('div');
    let head = document.createElement('h4');
    let list = document.createElement('x-emoji-list');

    tabs.addEventListener('click', ev =>
        root.dataset.category = Array.from(ev.target.parentElement.children).indexOf(ev.target));

    new MutationObserver(_ => {
        for (let tab of tabs.children)
            tab.classList.remove('active');
        tabs.children[root.dataset.category].classList.add('active');
        head.textContent = $.emoji.categories[root.dataset.category].name;
        list.innerHTML = $.emoji.categories[root.dataset.category].emoji.map($.emoji.wrap).join('');
    }).observe(root, {attributes: true, attributeFilter: ['data-category']});

    $.rules['[data-scrollbar]'](wrap);
    wrap.dataset.scrollbar = '';
    ...
    $.emoji.load().then(_ => {
        tabs.innerHTML = $.emoji.categories.map(c => `<i>${c.emoji[0]}</i>`).join('');
        root.dataset.status = '';
        root.dataset.category = root.dataset.category || '0';
    }).catch(err => {
        console.error(err);
...
$.emoji.wrap = m => {
    ...
    return `<span class="emoji" alt="${m}">${m}</span>`;
};
...
$.emoji.load = () => new Promise((resolve, reject) => {
    ...
    xhr.onload = xhr.onerror = ev => {
        ...
        setTimeout(resolve, 50);  // allow some time to render a frame before creating hundreds of nodes
    };
